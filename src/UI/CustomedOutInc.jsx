...
function CustomedOutInc() {
  const { dataInc, Change, today } = useConFast();
  Change("incomes");
  const dataIncFiltered = dataInc.filter((item) => item.date === today);
  return (
    <>
      <span className=" flex flex-col mr-2  md:mx-auto mx-0  mt-6 text-center w-3/6 md:w-[49%] h-[220px] bg-gray-300  rounded-[40px] p-5 text-2xl mb-2">
